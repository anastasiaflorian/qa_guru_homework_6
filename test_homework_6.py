...
def test_dark_theme_by_time_and_user_choice():
    ...
    (используется переключение по времени системы)
    """
...
def print_name(func, **kwargs):
    function_name = func.__name__.replace('_', ' ').title()
    arguments = [f"{value}" for key, value in kwargs.items()]
    arguments_string = ', '.join(arguments)
    print(f"{function_name} [{arguments_string}]")
...
def open_browser(browser_name):
    actual_result = print_name(open_browser, browser_name=browser_name)
    assert actual_result == "Open Browser [Chrome]"


def go_to_companyname_homepage(page_url):
    actual_result = print_name(go_to_companyname_homepage, page_url=page_url)
    assert actual_result == "Go To Companyname Homepage [https://companyname.com]"


def find_registration_button_on_login_page(page_url, button_text):
    actual_result = print_name(find_registration_button_on_login_page, page_url=page_url, button_text=button_text)
